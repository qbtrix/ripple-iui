 ...
 */
export function isQuizPattern(spec, items) {
    if (spec.intent !== 'select')
        return false;
    if (items.length === 0)
        return false;
 ...
 */
export function isResultsPattern(spec, items) {
    if (spec.intent !== 'info')
        return false;
    if (items.length === 0)
        return false;
 ...
 */
export function toQuizOptions(items, fields) {
 ...
 */
export function toResultsItems(items) {
    return items.map((item) => ({
 ...
 */
export function isChartPattern(spec, items) {
    // Explicit chart type in display hints
    if (spec.display?.chart_type)
        return true;
 ...
 */
export function toChartData(items, fields) {
