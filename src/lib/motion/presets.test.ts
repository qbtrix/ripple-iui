...
describe('motion presets', () => {
  it('instant = tween 100ms standard', () => {
    expect(resolvePreset('instant')).toEqual({ type: 'tween', duration: 100, easing: 'standard' });
  });
  ...
  it('smooth = tween 250ms decelerate', () => {
    expect(resolvePreset('smooth')).toEqual({ type: 'tween', duration: 250, easing: 'decelerate' });
  });
  it('gentle = tween 400ms standard', () => {
    expect(resolvePreset('gentle')).toEqual({ type: 'tween', duration: 400, easing: 'standard' });
  });
