...
<script lang="ts">
  ...
  $effect(() => {
    if (steps.length > 0 && (currentStep === undefined || !steps.some((s) => s.id === currentStep))) {
...
<div {id} class={cn('rwizard', orientation === 'vertical' && 'rwizard-vertical', className)} style={styleString}>
  {#if title || description}
    <header class="rwizard-header">
      {#if title}<h2 class="rwizard-title">{title}</h2>{/if}
      {#if description}<p class="rwizard-description">{description}</p>{/if}
    </header>
  {/if}

  {#if steps.length > 0}
    <ol class={cn('rwizard-steps', orientation === 'vertical' ? 'rwizard-steps-vertical' : 'rwizard-steps-horizontal')}>
      {#each steps as s, i}
          ...
          >
            ...
            <span class="rwizard-step-text">
              <span class="rwizard-step-label">
                {s.label}
                {#if s.optional}<span class="rwizard-step-optional">Optional</span>{/if}
              </span>
              {#if s.description}<span class="rwizard-step-desc">{s.description}</span>{/if}
            </span>
          </button>
          {#if i < steps.length - 1}
  ...
  <div class="rwizard-bar">
    <div>
      {#if showCancel}
        <button type="button" class="rwizard-btn rwizard-btn-ghost" onclick={handleCancel}>
          {cancelLabel}
        </button>
      {/if}
    </div>
    <div class="rwizard-bar-meta">
      <span class="rwizard-bar-pos">Step {currentIdx + 1} of {steps.length || 1}</span>
    </div>
...
<style>
  ...
  .rwizard-step-active .rwizard-step-pip {
    ...
    box-shadow: 0 0 0 4px color-mix(in oklab, oklch(0.55 0.18 250) 18%, transparent);
  }
