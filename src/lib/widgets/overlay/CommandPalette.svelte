...
<script lang="ts">
  ...
  function score(cmd: Command, q: string): number {
    if (!q) return 1;
    const haystack = [cmd.label, cmd.description ?? '', ...(cmd.keywords ?? []), cmd.group ?? '']
      .join(' ')
      .toLowerCase();
  ...
  >
    ...
    <div class="max-h-[360px] overflow-y-auto p-1">
      {#if filtered.length === 0}
        <div class="px-3 py-8 text-center text-sm text-muted-foreground">{emptyText}</div>
      {:else}
          ...
          {#each cmds as cmd (cmd.id)}
            ...
            >
              ...
              <span class="flex-1 min-w-0">
                ...
                {#if cmd.description}
                  <span class="block truncate text-xs text-muted-foreground">{cmd.description}</span>
                {/if}
              </span>
              {#if cmd.shortcut}
                <span class="ml-auto text-xs tracking-widest text-muted-foreground">{cmd.shortcut}</span>
              {/if}
            </button>
          {/each}
        {/each}
