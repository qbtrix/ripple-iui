...
describe('RippleHeadless', () => {
	...
	it('accepts an injected store', async () => {
		// The runtime depends on `StateStore`, never on a concrete class, so a
		// host can supply its own. The Svelte package proves the same thing with
		// its RUNE store (headless-with-runes.test.ts) — that assertion cannot
		// live here, because @ripple-ui/core has no Svelte compiler by design.
		const store = new HeadlessStateManager({ count: 10 });
