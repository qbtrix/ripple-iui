 ...
 * action; another renderer passes its own. The engine only needs the shape.
 */
...
export class EventDispatcher {
	...
	constructor(
		...
		/**
		 * Plays a motion on a resolved DOM node. Injected rather than imported:
		 * the player lives in a renderer (it is a Svelte action in
		 * `@ripple-ui/svelte`), and @ripple-ui/core must not reach into one.
		 * Optional and lazily supplied, so the dispatcher keeps working when no
		 * renderer is present — `animate` then emits its event and skips the
	...
	private handleAnimate(
		...
		if (root && target && handler.motion) {
			...
			if (node && this.playMotion) {
				// Injected by the renderer. Awaiting is deliberate-free: the pulse is
				// fire-and-forget, exactly as the old lazy-import call was.
				void this.playMotion(node, handler.motion);
			}
		}
