<!--
  ...
  needing to route the default through `cn` at all (twMerge dedupes classes in one
  class string; it cannot dedupe a class against a CSS rule, which is what this is).
  A passed explicit margin is also detected and skips the default outright as a belt-and-suspenders.
...
<style>
  /* Wrapper carries the default rhythm. `:where()` keeps specificity at 0 so any
     margin utility the spec passes through `class` still wins. twMerge in `cn` does
     not help here: the default is a CSS rule, not a class it could dedupe against. */
  .ripple-separator {
    display: contents;
  }
